...
!!! info "💡 Le type le plus courant: `inline-size`"
     **`container-type: inline-size`**: le plus courant. On mesure seulement la largeur (l'axe inline). 
     Les autres type de conteneur sont `size` (largeur ET hauteur) et `normal` (pas de mesure, mais le nom est disponible pour du style conditionnel).
...
- `cqi` : container query inline largeur totale du conteneur
