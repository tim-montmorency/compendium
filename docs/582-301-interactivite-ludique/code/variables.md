...
!!! tip "En JS"
    En JS, une variable (`let x = 5;`) peut contenir n'importe quel type, et ça peut changer. En C#, chaque variable a un **type fixe**, écrit avant son nom (`int x = 5;`). Une fois déclarée `int`, elle ne pourra jamais contenir autre chose qu'un nombre entier.

## Types de base

| Type | Stocke | Exemple |
| ---- | ------ | ------- |
| `int` | un nombre entier | `public int chickenQty = 10;` |
| `float` | un nombre décimal | `public float myFloat = 1.5f;` |
| `bool` | `true` ou `false` | `public bool myBool = true;` |
| `Vector2` | une coordonnée 2D (`x`, `y`) | `public Vector2 myVector2 = new Vector2(1f, 2f);` |
| `Vector3` | une coordonnée 3D (`x`, `y`, `z`) | `public Vector3 myVector3 = new Vector3(4f, 22f, 1f);` |

!!! note "Pourquoi le `f` après les décimaux ?"
    En C#, un nombre décimal comme `1.5` est un `double` par défaut. Pour un `float`, on doit écrire `1.5f`.
...
```csharp
if (myGameObject.CompareTag("tag")) { } // Vérifie le tag
myGameObject.SetActive(false);          // Désactive le GameObject
myGameObject.transform.position = new Vector3(0f, 0f, 0f); // Change sa position
```
...
```csharp
myTransform.position = new Vector3(0f, 0f, 0f);        // Position
myTransform.rotation = Quaternion.Euler(90f, 0f, 0f);   // Rotation
myTransform.localScale = new Vector3(2f, 2f, 2f);       // Scale
```

!!! tip "`transform`"
    Chaque script a déjà accès au `Transform` de son propre GameObject via `transform` (sans variable à déclarer) : `transform.position = ...`.
...
```csharp
renduVisuel.sprite = portraitHero2;                    // Change l'image (ex. ramasser une armure)
renduVisuel.color = new Color(0f, 0.5f, 1f, 0.5f);      // Teinte l'image (ici bleu semi-transparent)
renduVisuel.flipX = true;                               // Retourne l'image horizontalement
```
