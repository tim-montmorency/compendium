...
!!! warning "Division : int vs float (piège venant de JS)"
    En JS, `5 / 2` donne toujours `2.5`. En C#, si `a` et `b` sont des `int`, `a / b` fait une **division entière** (le résultat est arrondi vers le bas, donc `2`).
