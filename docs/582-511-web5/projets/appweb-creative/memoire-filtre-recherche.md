...
Ajoutez le `getters` nommé `filteredMemories` qui vous permettra d'afficher la dans salle les mémires filtrées (donc vous allez faire un `v-for`sur `filteredMemories` pour dynamiser votre affichage.
